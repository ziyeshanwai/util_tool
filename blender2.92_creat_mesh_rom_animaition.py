...
if __name__ == "__main__":
    print("--"*100)
    armature = bpy.data.objects["origin_animation"]
    ob_tar = bpy.data.objects["pose_0"]
    ob_source = bpy.data.objects["head_lod0_mesh_origin"]
    face = get_face_list(ob_tar.data)
    v = get_verts(ob_source)
    creat_object("pose_16", v, face)
    print_error(ob_tar, ob_source)
    print_error2(ob_tar, ob_source)
    print_error2(ob_tar, bpy.data.objects["pose_16"])
    print_error2(bpy.data.objects["pose_16"], ob_tar)
